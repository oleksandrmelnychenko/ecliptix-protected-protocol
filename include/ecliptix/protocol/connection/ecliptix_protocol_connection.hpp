...
public:
     ...
     *
     * @return Result containing unique_ptr to the connection or an error
     */
    [[nodiscard]] static Result<std::unique_ptr<EcliptixProtocolConnection>, EcliptixProtocolFailure> Create(
        uint32_t connection_id,
        bool is_initiator,
     ...
     *    - Output: 64 bytes → [new_root_key (32) | new_recv_chain_key (32)]
...
private:
    ...
    /**
     * @brief Perform DH ratchet operation
     *
     * @param is_sender True if performing sender ratchet, false for receiver
     * @param received_dh_public_key Peer's new DH public key (for receiver ratchet)
     * @return Ok(Unit) on success
     */
    [[nodiscard]] Result<Unit, EcliptixProtocolFailure>
        PerformDhRatchet(bool is_sender, std::span<const uint8_t> received_dh_public_key = {});

    /**
     * @brief Cleanup old keys if threshold exceeded
     */
