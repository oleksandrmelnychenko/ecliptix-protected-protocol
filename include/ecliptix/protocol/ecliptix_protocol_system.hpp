...
public:
    ...
    SendMessage(std::span<const uint8_t> payload);
    [[nodiscard]] Result<std::vector<uint8_t>, EcliptixProtocolFailure>
    ReceiveMessage(const proto::common::SecureEnvelope& envelope);
...
private:
    ...
    HandleDhRatchetIfNeeded(const proto::common::SecureEnvelope& envelope);
