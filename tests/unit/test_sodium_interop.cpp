...
TEST_CASE("SodiumInterop - Secure Wipe", "[sodium][crypto][security]") {
    ...
    SECTION("Wipe small buffer") {
        ...
        REQUIRE(result.IsOk());
        // Note: We can't verify buffer is zeroed from outside
        // (that would defeat the purpose), but we verify no errors
    }
    ...
    SECTION("Wipe too-large buffer fails") {
        // We can't actually allocate MAX_BUFFER_SIZE, so we test validation
        // by checking the constant exists
...
TEST_CASE("SodiumInterop - X25519 Key Generation", "[sodium][crypto][keygen]") {
    ...
    SECTION("Generate valid key pair") {
        ...
        // Check sizes
        REQUIRE(sk_handle.Size() == Constants::X_25519_PRIVATE_KEY_SIZE);
        REQUIRE(pk_bytes.size() == Constants::X_25519_PUBLIC_KEY_SIZE);
...
TEST_CASE("SodiumInterop - Ed25519 Key Generation", "[sodium][crypto][keygen]") {
    ...
    SECTION("Generate valid key pair") {
        ...
        // Check sizes
        REQUIRE(sk.size() == Constants::ED_25519_SECRET_KEY_SIZE);
        REQUIRE(pk.size() == Constants::ED_25519_PUBLIC_KEY_SIZE);
    }
