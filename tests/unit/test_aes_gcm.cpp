...
TEST_CASE("AES-GCM - Basic encryption and decryption", "[aes_gcm]") {
    ...
    SECTION("Encrypt and decrypt round-trip") {
        std::vector<uint8_t> key(Constants::AES_KEY_SIZE, 0xAA);
        std::vector<uint8_t> nonce(Constants::AES_GCM_NONCE_SIZE, 0xBB);
    ...
    SECTION("Empty plaintext") {
        std::vector<uint8_t> key(Constants::AES_KEY_SIZE, 0x11);
        std::vector<uint8_t> nonce(Constants::AES_GCM_NONCE_SIZE, 0x22);
        std::vector<uint8_t> plaintext = {};
        auto encrypt_result = AesGcm::Encrypt(key, nonce, plaintext);
        REQUIRE(encrypt_result.IsOk());
        auto ciphertext = encrypt_result.Unwrap();
        REQUIRE(ciphertext.size() == Constants::AES_GCM_TAG_SIZE);
        auto decrypt_result = AesGcm::Decrypt(key, nonce, ciphertext);
        REQUIRE(decrypt_result.IsOk());
        REQUIRE(decrypt_result.Unwrap().empty());
    }
    SECTION("Large plaintext") {
        std::vector<uint8_t> key(Constants::AES_KEY_SIZE, 0x33);
        std::vector<uint8_t> nonce(Constants::AES_GCM_NONCE_SIZE, 0x44);
...
TEST_CASE("AES-GCM - Authentication failures", "[aes_gcm]") {
    REQUIRE(SodiumInterop::Initialize().IsOk());
    std::vector<uint8_t> key(Constants::AES_KEY_SIZE, 0x66);
    std::vector<uint8_t> nonce(Constants::AES_GCM_NONCE_SIZE, 0x77);
    ...
    auto ciphertext = encrypt_result.Unwrap();
    SECTION("Wrong key") {
        std::vector<uint8_t> wrong_key(Constants::AES_KEY_SIZE, 0x99);
        auto result = AesGcm::Decrypt(wrong_key, nonce, ciphertext, ad);
        REQUIRE(result.IsErr());
    }
    SECTION("Wrong nonce") {
        std::vector<uint8_t> wrong_nonce(Constants::AES_GCM_NONCE_SIZE, 0x88);
...
TEST_CASE("AES-GCM - Input validation", "[aes_gcm]") {
    ...
    std::vector<uint8_t> valid_nonce(Constants::AES_GCM_NONCE_SIZE, 0xBB);
...
TEST_CASE("AES-GCM - Associated data variations", "[aes_gcm]") {
    REQUIRE(SodiumInterop::Initialize().IsOk());
    std::vector<uint8_t> key(Constants::AES_KEY_SIZE, 0xFF);
    std::vector<uint8_t> nonce(Constants::AES_GCM_NONCE_SIZE, 0xEE);
