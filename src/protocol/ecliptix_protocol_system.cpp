...
EcliptixProtocolSystem::SendMessage(std::span<const uint8_t> payload) {
    ...
    try {
        ...
        if (include_dh) {
            ...
            auto dh_option = dh_result.Unwrap();
            if (dh_option.has_value()) {
        ...
        auto encrypt_result = ratchet_key.WithKeyMaterial<std::vector<uint8_t>>(
            [&payload, &nonce, &ad](std::span<const uint8_t> key_material) -> Result<std::vector<uint8_t>, EcliptixProtocolFailure> {
        ...
        int32_t result_code = static_cast<int32_t>(proto::common::EnvelopeResultCode::SUCCESS);
        ...
        { auto __wipe1 = SodiumInterop::SecureWipe(std::span<uint8_t>(nonce)); (void)__wipe1; }
        { auto __wipe2 = SodiumInterop::SecureWipe(std::span<uint8_t>(ad)); (void)__wipe2; }
        { auto __wipe3 = SodiumInterop::SecureWipe(std::span<uint8_t>(encrypted_payload)); (void)__wipe3; }
        { auto __wipe4 = SodiumInterop::SecureWipe(std::span<uint8_t>(sender_dh_key)); (void)__wipe4; }
        { auto __wipe5 = SodiumInterop::SecureWipe(std::span<uint8_t>(metadata_key)); (void)__wipe5; }
        return Result<proto::common::SecureEnvelope, EcliptixProtocolFailure>::Err(
...
EcliptixProtocolSystem::ReceiveMessage(const proto::common::SecureEnvelope& envelope) {
    EcliptixProtocolConnection* connection = GetConnectionSafe();
    if (!connection) {
    ...
    std::vector<uint8_t> encrypted_payload;
    try {
        auto ratchet_result = HandleDhRatchetIfNeeded(envelope);
        ...
        auto metadata = decrypt_metadata_result.Unwrap();
        ...
        if (replay_result.IsErr()) {
            { auto __wipe = SodiumInterop::SecureWipe(std::span<uint8_t>(payload_nonce)); (void)__wipe; }
            return Result<std::vector<uint8_t>, EcliptixProtocolFailure>::Err(
        ...
        auto decrypt_result = ratchet_key.WithKeyMaterial<std::vector<uint8_t>>(
            [&encrypted_payload, &payload_nonce, &ad](std::span<const uint8_t> key_material) -> Result<std::vector<uint8_t>, EcliptixProtocolFailure> {
        ...
        { auto __wipe1 = SodiumInterop::SecureWipe(std::span<uint8_t>(dh_public_key)); (void)__wipe1; }
        { auto __wipe2 = SodiumInterop::SecureWipe(std::span<uint8_t>(header_nonce)); (void)__wipe2; }
        { auto __wipe3 = SodiumInterop::SecureWipe(std::span<uint8_t>(metadata_key)); (void)__wipe3; }
        { auto __wipe4 = SodiumInterop::SecureWipe(std::span<uint8_t>(ad)); (void)__wipe4; }
        return Result<std::vector<uint8_t>, EcliptixProtocolFailure>::Err(
            EcliptixProtocolFailure::Generic(
...
EcliptixProtocolSystem::HandleDhRatchetIfNeeded(
    const proto::common::SecureEnvelope& envelope) {
    ...
    try {
        ...
        if (current_key_result.IsErr()) {
            { auto __wipe = SodiumInterop::SecureWipe(std::span<uint8_t>(received_dh_key)); (void)__wipe; }
            return Result<Unit, EcliptixProtocolFailure>::Err(
                current_key_result.UnwrapErr());
        }
        auto current_key_option = current_key_result.Unwrap();
        if (current_key_option.has_value()) {
