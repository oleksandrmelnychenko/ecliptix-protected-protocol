...
EcliptixProtocolConnection::Create(
    ...
    try {
        ...
        std::vector<uint8_t> temp_chain_key(Constants::X_25519_KEY_SIZE, static_cast<uint8_t>(ProtocolConstants::ZERO_VALUE));
...
EcliptixProtocolConnection::FinalizeChainAndDhKeys(
    ...
    try {
        ...
        auto root_derive_result = Hkdf::DeriveKeyBytes(
            ...
            std::vector<uint8_t>(ProtocolConstants::DH_RATCHET_INFO.begin(), ProtocolConstants::DH_RATCHET_INFO.end()));
        ...
        auto sender_chain_result = Hkdf::DeriveKeyBytes(
            ...
            std::vector<uint8_t>(), 
            std::vector<uint8_t>(ProtocolConstants::INITIAL_SENDER_CHAIN_INFO.begin(), ProtocolConstants::INITIAL_SENDER_CHAIN_INFO.end()));
        ...
        auto receiver_chain_result = Hkdf::DeriveKeyBytes(
            ...
            Constants::X_25519_KEY_SIZE,
            std::vector<uint8_t>(), 
            std::vector<uint8_t>(ProtocolConstants::INITIAL_RECEIVER_CHAIN_INFO.begin(), ProtocolConstants::INITIAL_RECEIVER_CHAIN_INFO.end()));
...
EcliptixProtocolConnection::DeriveMetadataEncryptionKey() {
    ...
    auto metadata_key_result = Hkdf::DeriveKeyBytes(
        ...
        std::vector<uint8_t>(), 
        std::vector<uint8_t>(ProtocolConstants::METADATA_ENCRYPTION_INFO.begin(), ProtocolConstants::METADATA_ENCRYPTION_INFO.end()));
...
EcliptixProtocolConnection::GenerateNextNonce() {
    ...
    for (size_t i = ProtocolConstants::ZERO_VALUE; i < COUNTER_SIZE; ++i) {
        nonce[RANDOM_SIZE + i] = static_cast<uint8_t>((counter >> (i * ComparisonConstants::BIT_SHIFT_BYTE)) & ComparisonConstants::BYTE_MASK);
    }
