...
namespace ecliptix::protocol::chain_step {
    ...
    namespace {
        template<typename T>
        Result<T, ProtocolFailure> ConvertSodiumResult(Result<T, SodiumFailure> &&result) {
            if (result.IsOk()) {
                return Result<T, ProtocolFailure>::Ok(std::move(result).Unwrap());
            }
            return Result<T, ProtocolFailure>::Err(
                ProtocolFailure::Generic("Sodium operation failed"));
        }
    }
    ...
    Result<ChainStep, ProtocolFailure> ChainStep::Create(
        ...
        if (auto validate_result = ValidateChainKey(initial_chain_key); validate_result.IsErr()) {
            return Result<ChainStep, ProtocolFailure>::Err(validate_result.UnwrapErr());
        }
        ...
        if (chain_key_result.IsErr()) {
            return Result<ChainStep, ProtocolFailure>::Err(chain_key_result.UnwrapErr());
        }
        ...
        if (auto write_result = chain_key_handle.Write(initial_chain_key); write_result.IsErr()) {
            return Result<ChainStep, ProtocolFailure>::Err(
                ProtocolFailure::Generic("Failed to write chain key to secure memory"));
        }
        std::optional<SecureMemoryHandle> dh_private_key_handle;
        if (dh_private_key.has_value()) {
            if (dh_private_key->size() != Constants::X_25519_PRIVATE_KEY_SIZE) {
                return Result<ChainStep, ProtocolFailure>::Err(
                    ProtocolFailure::InvalidInput("DH private key must be 32 bytes"));
            }
            ...
            if (dh_sk_result.IsErr()) {
                return Result<ChainStep, ProtocolFailure>::Err(dh_sk_result.UnwrapErr());
            }
            ...
            if (auto dh_write_result = dh_sk_handle.Write(*dh_private_key); dh_write_result.IsErr()) {
                return Result<ChainStep, ProtocolFailure>::Err(
                    ProtocolFailure::Generic("Failed to write DH private key to secure memory"));
            }
        ...
        if (dh_public_key.has_value()) {
            if (dh_public_key->size() != Constants::X_25519_PUBLIC_KEY_SIZE) {
                return Result<ChainStep, ProtocolFailure>::Err(
                    ProtocolFailure::InvalidInput("DH public key must be 32 bytes"));
            }
            dh_public_key_copy = std::vector(dh_public_key->begin(), dh_public_key->end());
        }
        return Result<ChainStep, ProtocolFailure>::Ok(
            ChainStep(
                step_type,
    ...
    Result<ChainStep, ProtocolFailure> ChainStep::FromProtoState(
        ChainStepType step_type,
        const ChainStepState &proto) {
        if (proto.chain_key().size() != Constants::X_25519_KEY_SIZE) {
            return Result<ChainStep, ProtocolFailure>::Err(
                ProtocolFailure::InvalidInput("Invalid chain key size in protobuf state"));
        }
        ...
        if (!proto.dh_private_key().empty()) {
            if (proto.dh_private_key().size() != Constants::X_25519_PRIVATE_KEY_SIZE) {
                ...
                return Result<ChainStep, ProtocolFailure>::Err(
                    ProtocolFailure::InvalidInput("Invalid DH private key size in protobuf state"));
            }
        ...
        if (!proto.dh_public_key().empty()) {
            if (proto.dh_public_key().size() != Constants::X_25519_PUBLIC_KEY_SIZE) {
                ...
                return Result<ChainStep, ProtocolFailure>::Err(
                    ProtocolFailure::InvalidInput("Invalid DH public key size in protobuf state"));
            }
        ...
        if (auto set_index_result = chain_step.SetCurrentIndex(proto.current_index()); set_index_result.IsErr()) {
            return Result<ChainStep, ProtocolFailure>::Err(set_index_result.UnwrapErr());
        }
        return Result<ChainStep, ProtocolFailure>::Ok(std::move(chain_step));
    }

    Result<Unit, ProtocolFailure> ChainStep::DeriveNextChainKeys(
        ...
        if (current_chain_key.size() != Constants::X_25519_KEY_SIZE) {
            return Result<Unit, ProtocolFailure>::Err(
                ProtocolFailure::InvalidInput("Chain key must be 32 bytes"));
        }
        if (next_chain_key.size() != Constants::X_25519_KEY_SIZE) {
            return Result<Unit, ProtocolFailure>::Err(
                ProtocolFailure::InvalidInput("Next chain key buffer must be 32 bytes"));
        }
        if (message_key.size() != Constants::X_25519_KEY_SIZE) {
            return Result<Unit, ProtocolFailure>::Err(
                ProtocolFailure::InvalidInput("Message key buffer must be 32 bytes"));
        }
    ...
    Result<Unit, ProtocolFailure> ChainStep::ExecuteWithKey(
        uint32_t index,
        std::function<Result<Unit, ProtocolFailure>(std::span<const uint8_t>)> operation) {
        std::lock_guard guard(*lock_);
        TRY_UNIT(EnsureNotDisposed());
        if (auto cached_key_opt = TakeCachedMessageKey(index); cached_key_opt.has_value()) {
            ...
            if (key_bytes_result.IsErr()) {
                return Result<Unit, ProtocolFailure>::Err(key_bytes_result.UnwrapErr());
            }
        ...
        if (index + 1 == current_index_ || index == current_index_) {
            ...
            if (chain_key_result.IsErr()) {
                return Result<Unit, ProtocolFailure>::Err(chain_key_result.UnwrapErr());
            }
            ...
            if (write_result.IsErr()) {
                SodiumInterop::SecureWipe(std::span(message_key));
                return Result<Unit, ProtocolFailure>::Err(
                    ProtocolFailure::Generic("Failed to update chain key"));
            }
        ...
        if (index >= current_index_) {
            return Result<Unit, ProtocolFailure>::Err(
                ProtocolFailure::Generic("Cannot access future key - call SkipKeysUntil first"));
        }
        return Result<Unit, ProtocolFailure>::Err(
            ProtocolFailure::Generic("Key not available - already used or pruned"));
    }

    Result<ChainKey, ProtocolFailure>
    ChainStep::GetOrDeriveKeyFor(const uint32_t index) {
        std::lock_guard guard(*lock_);
        if (auto disposed_check = EnsureNotDisposed(); disposed_check.IsErr()) {
            return Result<ChainKey, ProtocolFailure>::Err(disposed_check.UnwrapErr());
        }
        ...
        if (index > current_index_) {
            if (auto skip_result = SkipKeysUntilLocked(index); skip_result.IsErr()) {
                return Result<ChainKey, ProtocolFailure>::Err(skip_result.UnwrapErr());
            }
        } else if (index == current_index_) {
            current_index_++;
        }
        return Result<ChainKey, ProtocolFailure>::Ok(
            ChainKey(this, index)
        );
    }

    Result<std::vector<uint8_t>, ProtocolFailure> ChainStep::GetCurrentChainKey() const {
        std::lock_guard guard(*lock_);
        if (auto disposed_check = EnsureNotDisposed(); disposed_check.IsErr()) {
        ...
        if (bytes_result.IsErr()) {
            return Result<std::vector<uint8_t>, ProtocolFailure>::Err(bytes_result.UnwrapErr());
        }
        return Result<std::vector<uint8_t>, ProtocolFailure>::Ok(std::move(bytes_result).Unwrap());
    }

    Result<uint32_t, ProtocolFailure> ChainStep::GetCurrentIndex() const {
        std::lock_guard guard(*lock_);
        if (auto disposed_check = EnsureNotDisposed(); disposed_check.IsErr()) {
            return Result<uint32_t, ProtocolFailure>::Err(disposed_check.UnwrapErr());
        }
        return Result<uint32_t, ProtocolFailure>::Ok(current_index_);
    }

    Result<Unit, ProtocolFailure> ChainStep::SetCurrentIndex(const uint32_t new_index) {
        std::lock_guard guard(*lock_);
        if (auto disposed_check = EnsureNotDisposed(); disposed_check.IsErr()) {
    ...
    Result<Unit, ProtocolFailure> ChainStep::SkipKeysUntil(const uint32_t target_index) {
        std::lock_guard guard(*lock_);
        if (auto disposed_check = EnsureNotDisposed(); disposed_check.IsErr()) {
            return disposed_check;
        }
    ...
    Result<Unit, ProtocolFailure> ChainStep::SkipKeysUntilLocked(uint32_t target_index) {
        ...
        if (uint32_t skip_count = target_index - current_index_;
            ...
            return Result<Unit, ProtocolFailure>::Err(
                ProtocolFailure::Generic("Skip gap too large - possible DoS attempt"));
        }
        auto chain_key_result = ConvertSodiumResult(chain_key_handle_.ReadBytes(Constants::X_25519_KEY_SIZE));
        if (chain_key_result.IsErr()) {
            return Result<Unit, ProtocolFailure>::Err(chain_key_result.UnwrapErr());
        ...
        if (write_result.IsErr()) {
            return Result<Unit, ProtocolFailure>::Err(
                ProtocolFailure::Generic("Failed to update chain key after skip"));
        }
        current_index_ = target_index;
        return Result<Unit, ProtocolFailure>::Ok(Unit{});
    }

    void ChainStep::PruneOldKeys() {
        std::lock_guard guard(*lock_);
        if (disposed_) {
            return;
        }
    ...
    Result<Unit, ProtocolFailure> ChainStep::UpdateKeysAfterDhRatchet(
        ...
        if (auto write_result = chain_key_handle_.Write(new_chain_key); write_result.IsErr()) {
            return Result<Unit, ProtocolFailure>::Err(
                ProtocolFailure::Generic("Failed to write new chain key"));
        }
        ...
        if (new_dh_private_key.has_value()) {
            ...
            if (handle_result.IsErr()) {
                return Result<Unit, ProtocolFailure>::Err(handle_result.UnwrapErr());
            }
            ...
            if (auto new_write_result = new_handle.Write(*new_dh_private_key); new_write_result.IsErr()) {
                return Result<Unit, ProtocolFailure>::Err(
                    ProtocolFailure::Generic("Failed to write new DH private key"));
            }
    ...
    Result<Option<std::vector<uint8_t> >, ProtocolFailure> ChainStep::ReadDhPublicKey() const {
        std::lock_guard guard(*lock_);
        if (auto disposed_check = EnsureNotDisposed(); disposed_check.IsErr()) {
            return Result<Option<std::vector<uint8_t> >, ProtocolFailure>::Err(disposed_check.UnwrapErr());
        }
        if (!dh_public_key_.has_value()) {
            return Result<Option<std::vector<uint8_t> >, ProtocolFailure>::Ok(std::nullopt);
        }
        std::vector copy(*dh_public_key_);
        return Result<Option<std::vector<uint8_t> >, ProtocolFailure>::Ok(std::move(copy));
    }

    Option<const SecureMemoryHandle *> ChainStep::GetDhPrivateKeyHandle() const {
        std::lock_guard guard(*lock_);
    ...
    Result<ChainStepState, ProtocolFailure> ChainStep::ToProtoState() const {
        std::lock_guard guard(*lock_);
        if (auto disposed_check = EnsureNotDisposed(); disposed_check.IsErr()) {
            return Result<ChainStepState, ProtocolFailure>::Err(disposed_check.UnwrapErr());
        }
        ...
        if (chain_key_result.IsErr()) {
            return Result<ChainStepState, ProtocolFailure>::Err(
                ProtocolFailure::Generic("Failed to read chain key for serialization"));
        }
        ...
        if (dh_private_key_handle_.has_value()) {
            ...
            if (dh_sk_result.IsErr()) {
                return Result<ChainStepState, ProtocolFailure>::Err(
                    ProtocolFailure::Generic("Failed to read DH private key for serialization"));
            }
        ...
        return Result<ChainStepState, ProtocolFailure>::Ok(std::move(proto));
    }

    Result<Unit, ProtocolFailure> ChainStep::StoreMessageKey(
        ...
        if (const auto store_write_result = handle.Write(message_key); store_write_result.IsErr()) {
            return Result<Unit, ProtocolFailure>::Err(
                ProtocolFailure::Generic("Failed to store message key in cache"));
        }
        cached_message_keys_[index] = std::move(handle);
        return Result<Unit, ProtocolFailure>::Ok(Unit{});
    }

    Option<SecureMemoryHandle> ChainStep::TakeCachedMessageKey(const uint32_t index) {
    ...
    Result<Unit, ProtocolFailure> ChainStep::EnsureNotDisposed() const {
        if (disposed_) {
            return Result<Unit, ProtocolFailure>::Err(
                ProtocolFailure::Generic("ChainStep has been disposed"));
        }
        return Result<Unit, ProtocolFailure>::Ok(Unit{});
    }

    Result<Unit, ProtocolFailure> ChainStep::ValidateChainKey(
