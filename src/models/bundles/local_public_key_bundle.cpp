...
namespace ecliptix::protocol::models {
    LocalPublicKeyBundle::LocalPublicKeyBundle(
        ...
        const std::optional<uint32_t> used_one_time_pre_key_id)
        : identity_ed25519_public_(std::move(identity_ed25519_public))
          , identity_x25519_public_(std::move(identity_x25519_public))
          ...
          , used_one_time_pre_key_id_(used_one_time_pre_key_id) {
    }
}
