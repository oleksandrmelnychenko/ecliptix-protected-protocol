/**
 * @file epp_api.cpp
 * @brief Client-side C API implementation for EPP (Ecliptix Protection Protocol)
 *
 * This file contains client-specific API functions. Shared functions are
...
// Note: ProtocolConnectionHandle is client-specific (not used by server)
...
EppErrorCode epp_session_begin_handshake(
    ...
    if (bundle.HasEphemeralKey()) {
        ...
        fprintf(
            stderr,
            "[BEGIN-HANDSHAKE-KYBER] My ephemeral_x25519: %02x%02x%02x%02x%02x%02x%02x%02x (SHOULD NOT BE USED BY SERVER!)\n",
...
EppErrorCode epp_session_complete_handshake_auto(
    ...
    if (!peer_bundle.ephemeral_x25519_public_key().empty()) {
        fprintf(
            stderr, "[COMPLETE-HANDSHAKE-AUTO] Peer ephemeral_x25519: %02x%02x%02x%02x%02x%02x%02x%02x (size=%zu)\n",
    ...
    if (is_initiator) {
        ...
        if (ek_public.has_value() && ek_private_result.IsOk()) {
            ...
            fprintf(
                stderr,
                "[COMPLETE-HANDSHAKE-AUTO] Captured ephemeral key BEFORE X3DH (INITIATOR): %02x%02x%02x%02x%02x%02x%02x%02x\n",
        ...
        fprintf(
            stderr, "[COMPLETE-HANDSHAKE-AUTO] Using SPK as initial DH (RESPONDER): %02x%02x%02x%02x%02x%02x%02x%02x\n",
            initial_dh_public[0], initial_dh_public[1], initial_dh_public[2], initial_dh_public[3],
            initial_dh_public[4], initial_dh_public[5], initial_dh_public[6], initial_dh_public[7]);
    }
    ...
    if (kyber_artifacts_result.IsOk()) {
        fprintf(stderr, "[COMPLETE-HANDSHAKE-AUTO] Using pending Kyber artifacts from BeginHandshakeWithPeerKyber\n");
        ...
        fprintf(stderr, "[COMPLETE-HANDSHAKE-AUTO] Decapsulating peer's kyber_ciphertext (size=%zu)\n",
                peer_bundle.kyber_ciphertext().size());
        ...
        kyber_ciphertext = std::move(artifacts.kyber_ciphertext);
        kyber_shared_secret = std::move(artifacts.kyber_shared_secret);
        fprintf(stderr, "[COMPLETE-HANDSHAKE-AUTO] Decapsulation successful\n");
    } else {
        auto _wipe_root = SodiumInterop::SecureWipe(std::span(root_key));
        (void) _wipe_root;
    ...
    fprintf(stderr, "[COMPLETE-HANDSHAKE-AUTO] kyber_ss: %02x%02x%02x%02x%02x%02x%02x%02x\n",
            kyber_shared_secret[0], kyber_shared_secret[1],
            kyber_shared_secret[2], kyber_shared_secret[3],
            kyber_shared_secret[4], kyber_shared_secret[5],
            kyber_shared_secret[6], kyber_shared_secret[7]);
...
EppErrorCode epp_session_get_used_prekey_id(
    ...
    const auto selected_opk_id = handle->system->GetIdentityKeys().GetSelectedOpkId();
...
EppErrorCode epp_session_set_kyber_secrets(
    ...
    if (!kyber_ciphertext) {
        fill_error(out_error, EPP_ERROR_NULL_POINTER, "Kyber ciphertext is null");
        return EPP_ERROR_NULL_POINTER;
    }
    if (kyber_ciphertext_length == 0) {
        fill_error(out_error, EPP_ERROR_INVALID_INPUT, "Kyber ciphertext is empty");
        return EPP_ERROR_INVALID_INPUT;
    }
    if (!kyber_shared_secret) {
        fill_error(out_error, EPP_ERROR_NULL_POINTER, "Kyber shared secret is null");
        return EPP_ERROR_NULL_POINTER;
    }
    if (kyber_shared_secret_length == 0) {
        fill_error(out_error, EPP_ERROR_INVALID_INPUT, "Kyber shared secret is empty");
        return EPP_ERROR_INVALID_INPUT;
    }
...
EppErrorCode epp_session_deserialize(
    ...
    if (!state_bytes) {
        fill_error(out_error, EPP_ERROR_NULL_POINTER, "State bytes is null");
        return EPP_ERROR_NULL_POINTER;
    }
