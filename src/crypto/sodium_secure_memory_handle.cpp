...
namespace ecliptix::protocol::crypto {
    ...
    Result<Unit, SodiumFailure> SecureMemoryHandle::Write(const std::span<const uint8_t> data) const {
        if (IsInvalid()) {
    ...
    Result<std::vector<uint8_t>, SodiumFailure> SecureMemoryHandle::ReadBytes(const size_t size) const {
        if (IsInvalid()) {
